...
/**
 * Esta clase permite leer parámetros de un archivo de propiedades.
 * @author christian
 *
 */
...
//	 private static String ARCH_CONF="/usr/local/supermercado/bin/supermercado.properties";
	 ...
	 * @return El valor.
	 * @throws GWT_ExcepcionIO Si hay algún problema con el archivo de propiedades.
	 */
	public static  String obtener_valor(String clave) throws ExcepcionIO {
		...
		try {
			...
			// Deésta manera busca el archivo de conf en la ruta
			// WEB-INF/clases/archivo.properties
			// y éste se genera a partir del que está en la carpeta resources.
			
			FileInputStream inputStream = new FileInputStream(ARCH_CONF);

			// Deésta manera busca el archivo de conf en la ruta
			// WEB-INF/clases/ar/gov/mecon/properties/archivo.properties
			// y éste se genera a partir del que está en la carpeta 
...
//			InputStream inputStream = getClass().getClassLoader().getResourceAsStream(ARCH_CONF);
			...
			if (inputStream != null) {
				...
				throw new ExcepcionIO("El archivo de propiedades '" + 
						ARCH_CONF + "' no fue encontrado ó no se pudo leer." +
